...
def _orders_table(root):
    support.unit(root, "m_table", {"up.sql": (
        "CREATE TABLE orders (\n"
        "  id     NUMBER(10) NOT NULL PRIMARY KEY,\n"
        "  region VARCHAR2(2 CHAR)\n"
        ")"
    )})
    support.unit(root, "m_seed", {"up.sql": (
        "INSERT INTO orders (id, region) SELECT LEVEL, NULL FROM dual "
        "CONNECT BY LEVEL <= 12"
    )})
    return [
        {"id": "create-orders", "path": "m_table", "language": "sql",
         "mode": "restartable", "entry": "up.sql"},
        {"id": "seed-orders", "path": "m_seed", "language": "sql", "mode": "atomic",
         "entry": "up.sql"},
    ]
...
def test_bounded_backfill_through_the_facade(oracle_project, oracle_query):
    ...
    manifest = support.manifest(root, entries + [
        {"id": "backfill", "path": "m_fill", "language": "python", "mode": "restartable",
         "entry": "migration.py"},
    ])
...
def test_checkpoint_and_batch_commit_together_and_survive_a_failure(oracle_project,
                                                                   oracle_query):
    root, config, schema = oracle_project
    entries = _orders_table(root)
    ...
    assert oracle_query(
        "SELECT prog_key, prog_value FROM m8_progress"
    ) == [("last_id", "10")]
    assert oracle_query(
...
def test_batch_rollback_discards_data_and_checkpoint(oracle_project, oracle_query):
    root, config, schema = oracle_project
    entries = _orders_table(root)
    support.unit(root, "m_fill", {"migration.py": (
        "def migrate(ctx):\n"
        "    with ctx.transaction() as tx:\n"
...
def test_executemany_raises_on_error_instead_of_collecting_row_errors(oracle_project,
                                                                     oracle_query):
    root, config, schema = oracle_project
    entries = _orders_table(root)
    support.unit(root, "m_fill", {"migration.py": (
    ...
    manifest = support.manifest(root, entries + [
        {"id": "bulk", "path": "m_fill", "language": "python", "mode": "restartable",
         "entry": "migration.py"},
    ])
...
def test_dml_returning_is_not_exposed_as_a_query_result(oracle_project, oracle_query):
    ...
    support.unit(root, "m_fill", {"migration.py": (
        ...
        "        try:\n"
        "            rows = tx.query(\n"
        "                \"UPDATE orders SET region = 'EU' WHERE id = 1 RETURNING id INTO :out\"\n"
        "            )\n"
        "        except Exception as exc:\n"
        "            ctx.log('returning rejected', detail=str(exc)[:80])\n"
        "        else:\n"
        "            raise AssertionError(f'RETURNING must not yield a result set: {rows}')\n"
        "        tx.execute(\"UPDATE orders SET region = 'EU' WHERE id = 1\")\n"
        "        ctx.progress.set('done', 'yes')\n"
    )})
    manifest = support.manifest(root, entries + [
        {"id": "returning", "path": "m_fill", "language": "python",
...
def test_progress_value_accepts_non_ascii_within_the_byte_limit(oracle_project,
                                                                oracle_query):
    root, config, schema = oracle_project
    entries = _orders_table(root)
    support.unit(root, "m_fill", {"migration.py": (
    ...
    manifest = support.manifest(root, entries + [
        {"id": "progress", "path": "m_fill", "language": "python",
         "mode": "restartable", "entry": "migration.py"},
    ])
    assert support.migrate(config, manifest) == Exit.OK
...
def test_ddl_outside_a_batch_is_allowed_and_inside_is_not(oracle_project, oracle_query):
    ...
    manifest = support.manifest(root, [{
        "id": "ddl", "path": "m1", "language": "python", "mode": "restartable",
        "entry": "migration.py",
    }])
    assert support.migrate(config, manifest) == Exit.OK
...
def test_facade_refusals(oracle_project, oracle_query, body, expected):
    ...
    manifest = support.manifest(root, entries + [
        {"id": "bad", "path": "m_bad", "language": "python", "mode": "restartable",
         "entry": "migration.py"},
    ])
    report = support.migrate_report(config, manifest)
    assert report.exit_code == Exit.MIGRATION_FAILED
...
def test_ddl_with_an_open_transaction_is_refused(oracle_project, oracle_query):
    ...
    root, config, schema = oracle_project
    entries = _orders_table(root)
    support.unit(root, "m_bad", {"migration.py": (
        "def migrate(ctx):\n"
        "    with ctx.transaction() as tx:\n"
        "        tx.execute(\"UPDATE orders SET region = 'EU' WHERE id = 1\")\n"
    ...
    assert oracle_query(
        "SELECT id FROM orders WHERE region = 'EU' ORDER BY id"
    ) == [(1,)]
    assert oracle_query(
        "SELECT COUNT(*) FROM all_tables WHERE owner = :owner AND "
        "table_name = 'AFTER_OPEN_TXN'", owner=schema,
    ) == [(0,)]


def test_open_transaction_on_return_is_rolled_back_and_refused(oracle_project,
                                                              oracle_query):
    root, config, schema = oracle_project
    entries = _orders_table(root)
    support.unit(root, "m_bad", {"migration.py": (
    ...
    assert oracle_query("SELECT COUNT(*) FROM orders WHERE region IS NOT NULL") == [(0,)]
    assert oracle_query(
        "SELECT status FROM m8_history WHERE migration_id = 'bad'"
    ) == [("ACTIVE",)]


# --- Oracle SQL details ------------------------------------------------------------

@pytest.mark.parametrize("sql,probe", [
...
def test_literals_and_comments_survive_normalisation(oracle_project, oracle_query,
                                                     sql, probe):
    root, config, schema = oracle_project
    entries = _orders_table(root)
    support.unit(root, "m_sql", {"up.sql": sql})
    manifest = support.manifest(root, entries + [
        {"id": "detail", "path": "m_sql", "language": "sql", "mode": "atomic",
         "entry": "up.sql"},
    ])
    assert support.migrate(config, manifest) == Exit.OK
...
def test_multiline_update_with_a_set_line_and_a_slash_literal(oracle_project,
                                                              oracle_query):
    root, config, schema = oracle_project
    entries = _orders_table(root)
    support.unit(root, "m_sql", {"up.sql": (
        "UPDATE orders\n"
        "SET region = 'EU'\n"
        "WHERE id = 1\n"
    )})
...
def test_stored_plsql_keeps_its_terminator_and_create_library_does_not(oracle_project,
                                                                      oracle_query):
    root, config, schema = oracle_project
    ...
    support.unit(root, "m2", {"up.sql": (
        "CREATE OR REPLACE LIBRARY lib_probe AS '/nonexistent/libprobe.so';\n"
    )})
    manifest = support.manifest(root, [
        {"id": "func", "path": "m1", "language": "sql", "mode": "restartable",
         "entry": "up.sql", "require_valid": [{"name": "F_REGION", "type": "FUNCTION"}]},
        {"id": "lib", "path": "m2", "language": "sql", "mode": "restartable",
         "entry": "up.sql"},
    ])
    assert support.migrate(config, manifest) == Exit.OK
    ...
    assert oracle_query(
        ...
        "object_name = 'LIB_PROBE' AND object_type = 'LIBRARY'", owner=schema,
    ) == [(1,)]


def test_sqlplus_command_in_a_sql_entry_is_refused_before_execution(oracle_project,
                                                                   oracle_query):
    root, config, schema = oracle_project
    support.unit(root, "m1", {"up.sql": "SET SERVEROUTPUT ON\n"})
    manifest = support.manifest(root, [{
        "id": "bad", "path": "m1", "language": "sql", "mode": "restartable",
        "entry": "up.sql",
    }])
    assert support.migrate(config, manifest) == Exit.VALIDATION
...
def test_sql_to_python_recovery_records_consistent_metadata(oracle_project, oracle_query):
    ...
    entry = {
        "id": "convert", "path": "m1", "language": "sql", "mode": "restartable",
        "entry": "up.sql",
    ...
    assert support.migrate(config, manifest) == Exit.MIGRATION_FAILED
    before = oracle_query(
        "SELECT seq, \"MODE\", first_fingerprint, started_at FROM m8_history"
    )[0]
...
def test_recovery_converges_from_checkpoints_of_two_earlier_versions(oracle_project,
                                                                    oracle_query):
    ...
    assert oracle_query("SELECT COUNT(*) FROM m8_progress") == [(0,)]
    assert oracle_query(
        "SELECT attempt FROM m8_history WHERE migration_id = 'backfill'"
    ) == [(3,)]
...
def test_recover_cannot_change_mode(oracle_project, oracle_query):
    ...
    entry = {"id": "m", "path": "m1", "language": "python", "mode": "restartable",
             "entry": "migration.py"}
    manifest = support.manifest(root, [entry])
    assert support.migrate(config, manifest) == Exit.MIGRATION_FAILED
...
def test_connect_user_differs_from_target_schema(tmp_path, oracle_settings, oracle_query):
    ...
    with oracledb.connect(user=runner, password=oracle_settings["password"],
        ...
        for name, kind in cursor.execute(
            ...
            "AND object_type IN ('TABLE','VIEW') ORDER BY object_type DESC", owner=owner,
        ).fetchall():
    ...
    manifest = support.manifest(tmp_path, [{
        "id": "split", "path": "m1", "language": "sql", "mode": "restartable",
        "entry": "up.sql",
    }])
    assert support.migrate(config, manifest) == Exit.OK
    ...
    with oracledb.connect(user=runner, password=oracle_settings["password"],
                          dsn=oracle_settings["dsn"]) as probe:
        rows = probe.cursor().execute(
            "SELECT object_name FROM all_objects WHERE owner = :owner AND "
            "object_type = 'TABLE' ORDER BY object_name", owner=owner,
        ).fetchall()
        assert [row[0] for row in rows] == ["M8_HISTORY", "M8_META", "M8_PROGRESS",
                                            "SPLIT_T"]
        namespace = probe.cursor().execute(
            f'SELECT target_namespace FROM "{owner}".m8_meta'
        ).fetchone()
